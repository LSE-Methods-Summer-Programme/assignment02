...
head(cmpdataLong)
typeof(cmpdataLong$category)
cmpdataLong$category <- as.character(cmpdataLong$category)  # Why???
head(cmpdataLong)
typeof(cmpdataLong$category)
with(subset(cmpdataLong, grepl("^per7", category)), table(countryname, category))
```

b.  Try using **dplyr** on `cmpdataLong` to "spread" this back into the wide format.

__Note:__ **dplyr** is a grammar of data manipulation, part of the **tidyverse**.

```{r}
suppressPackageStartupMessages(library("dplyr"))
suppressPackageStartupMessages(library("tidyverse"))
spreadedData <- spread(cmpdataLong, key = "category", value = "catcount")
head(spreadedData)
# identical(spreadedData, cmpdata)
# They are not identical because of the selection of id.vars
```
